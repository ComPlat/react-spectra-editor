...
import { getLcMsInfo, splitAndReindexEntities } from './extractEntityLCMS';
...
const selectLcmsMsPage = (entities = [], request = {}) => {
  ...
  if (candidateEntries.length === 1) {
    ...
    const matchingEntries = candidateEntries.filter(({ entity }) => getLcMsInfo(entity).polarity === polarity);
...
const buildLcmsMsPageJcamp = (entities = [], request = {}) => {
  ...
  const {
    retentionTimeRaw, retentionTime, polarity, entity, entityIndex, spectrum, sourceFallback, pageFallback,
  } = selection;
...
const ExtractJcamp = (source) => {
  ...
  if (lcmsMzPageFromInfo != null) {
    return { spectra, features, layout, lcms_mz_page: lcmsMzPageFromInfo };
  }
