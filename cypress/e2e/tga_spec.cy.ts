describe('TGA', () => {
  beforeEach(() => {
    ...
    cy.get('#btn-tga').click()
  })

  it('Open layout', () => {
    cy.get('.d3Line').children().should('have.class', 'd3Svg')
    cy.get('.d3Svg text.xLabel').should('have.text', 'X (SECONDS)')
    cy.get('.d3Svg text.yLabel').should('have.text', 'Y (ARBITRARY UNITS)')
  })
  ...
  function removePeak(view) {
    ...
    cy.get('.btn-sv-bar-rmpeak').click()
      for (let index = 0; index < 10; index++) {
        cy.get('.d3Svg')
        .trigger('click', basePointX*index, 170, {
          which: 1,
          view: view,
        })
      }
