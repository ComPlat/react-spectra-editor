...
const onClickCb = (operation, peaksEdit, isAscend, isIntensity, scan, thres, layoutSt, shiftSt, analysis, decimalSt, integrationSt, multiplicitySt, allIntegrationSt, aucValues, waveLengthSt, cyclicvoltaSt, curveSt, axesUnitsSt, detectorSt, dscMetaData, hplcMsSt) => () => {
  ...
  let lcmsUvvisWavelength = null;
  let lcmsMzPage = null;
  let lcmsPeaksText = null;
  if (layoutSt === 'LC/MS') {
    ...
    lcmsUvvisWavelength = hplcMsSt?.uvvis?.selectedWaveLength ?? null;
    lcmsMzPage = hplcMsSt?.tic?.currentPageValue ?? null;
    lcmsPeaksText = _format.default.formatedLCMS(hplcMsSt, isAscend, decimalSt);
  }
  operation({
    peaks: peaksEdit,
    lcms_peaks_text: lcmsPeaksText,
    lcms_peaks: lcmsPeaks,
    lcms_integrals: lcmsIntegrals,
    lcms_uvvis_wavelength: lcmsUvvisWavelength,
