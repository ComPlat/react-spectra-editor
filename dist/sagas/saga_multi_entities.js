...
function* setSimulationForCurve(action) {
  // eslint-disable-line
  const layoutSt = yield (0, _effects.select)(getLayoutSt);
  if (!_format.default.isNmrLayout(layoutSt)) return;
  const curveSt = yield (0, _effects.select)(getCurveSt);
  const {
    listCurves,
    curveIdx
...
const multiEntitiesSagas = [(0, _effects.takeEvery)(_action_type.CURVE.SET_ALL_CURVES, setCyclicVoltametry), (0, _effects.takeEvery)(_action_type.CURVE.SET_ALL_CURVES, setInitIntegrations), (0, _effects.takeEvery)(_action_type.CURVE.SELECT_WORKING_CURVE, setSimulationForCurve), (0, _effects.takeEvery)(_action_type.CYCLIC_VOLTA_METRY.SET_FACTOR, setCyclicVoltametryRef)];
